...
import { PrinterConfig, ZplGenerationOptions } from '../types';

/**
 * ZPL (Zebra Programming Language) Generation Service.
 * Formats standards-compliant ZPL II instructions tailored specifically for
 * ZDesigner ZT230-200dpi ZPL label printers.
 * Calibrated for 30mm x 10mm labels with fixed 25mm x 6mm centered barcode and 3mm text.
 */
...
/**
 * Escapes characters that have special meaning in ZPL format definitions.
 */
export function escapeZplString(str: string): string {
  if (!str) return '';
  return str.replace(/\^/g, '_5E').replace(/~/g, '_7E');
}

/**
 * Generates single label ZPL code for a given barcode value and print parameters.
 * Automatically centers native Code 128 barcode (24x6mm, 100% scannable) and prints 3mm (24 dots) full text below on 30x10mm stickers.
 */
export function generateSingleLabelZpl(
  barcodeValue: string,
  options: Partial<ZplGenerationOptions> = {}
): string {
  const rawVal = (barcodeValue || '').trim();
  const escapedValue = escapeZplString(rawVal);
  const {
  ...
  const symbology = options.barcodeSymbology || 'DATAMATRIX';
  ...
  if (symbology === 'DATAMATRIX' && isSmallLabel) {
    ...
    return [
      ...
      `^FO${barcodeLeft},${topMargin}^BXN,2,200^FD${escapedValue}^FS`,
      // Full human-readable barcode text centered directly below (3mm / 24 dots height)
      `^FO0,${textTopMargin}^A0N,${fontHeight},${fontWidth}^FB${labelWidthDots},1,0,C,0^FD${escapedValue}^FS`,
      '^XZ',
    ].join('\n');
  }
  ...
  const escapedScannable = escapeZplString(scannableBarcode);
...
function generateColumnZpl(
  ...
  if (symbology === 'DATAMATRIX' && isSmallLabel) {
    ...
    return [
      `^FO${absBarcodeLeft},${topMargin}^BXN,2,200^FD${escapedValue}^FS`,
      `^FO${colLeftDots},${textTopMargin}^A0N,${fontHeight},${fontWidth}^FB${labelWidthDots},1,0,C,0^FD${escapedValue}^FS`,
    ].join('\n');
  }
  ...
  return [
    `^FO${absBarcodeLeft},${topMargin}^BY${modWidth},3,${barcodeHeight}^BCN,${barcodeHeight},N,N,N,A^FD${escapedValue}^FS`,
    `^FO${colLeftDots},${textTopMargin}^A0N,${fontHeight},${fontWidth}^FB${labelWidthDots},1,0,C,0^FD${escapedValue}^FS`,
  ].join('\n');
}

/**
 * Generates combined ZPL code for a batch of labels.
...
export function generateBatchZpl(
  ...
  if (columnsAcross <= 1) {
    const options: ZplGenerationOptions = {
      barcodeValue: '',
      labelWidthDots,
      labelHeightDots,
      dpi,
      barcodeHeightDots: config.barcodeHeightDots || 48,
      moduleWidthDots: config.moduleWidthDots || 1,
      humanReadableFontSizeDots: config.humanReadableFontSizeDots || 24,
      horizontalOffsetDots: config.horizontalOffsetDots || 0,
      verticalOffsetDots: config.verticalOffsetDots || 0,
      orientation,
    ...
    return validValues
      .map((val) => generateSingleLabelZpl(val, { ...options, barcodeValue: val }))
      .join('\n\n');
  }
  ...
  const grid = layoutGrid(validValues, columnsAcross, printDirection);
    ...
    .map((row) => {
      ...
      for (let c = 0; c < columnsAcross; c++) {
        ...
        if (val) {
          ...
          colZplParts.push(generateColumnZpl(val, colLeft, config));
        }
      }
...
/**
 * Generates calibration and alignment test label ZPL.
 * If multi-column (3-across) is enabled, prints calibration across all columns.
 */
export function generateTestLabelZpl(config: PrinterConfig): string {
  const columnsAcross = Math.max(1, config.columnsAcross || 1);
  ...
  if (columnsAcross > 1) {
    ...
    return generateBatchZpl(testValues, config);
  }
  ...
  if (isSmallLabel) {
    return generateSingleLabelZpl('ATM1-CDS1-11111-V1-IN11112', {
      ...config,
      labelWidthDots,
      labelHeightDots,
      dpi,
  ...
  return [
    ...
    `^FO${barcodeLeft},58^BY${modWidth},3,60^BCN,60,N,N,N,A^FD${testBarcode}^FS`,
    `^FO20,126^A0N,24,18^FB${labelWidthDots - 40},1,0,C,0^FD${testBarcode}^FS`,
